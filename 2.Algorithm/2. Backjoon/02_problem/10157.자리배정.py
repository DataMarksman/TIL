...
# 3. 실제 보드는 필요 없는게 아닐까? 좌표만 가지고 놀아보자~
# 개선점:
...
if K > M*N:                                                # 아니 글쎄, 저희가 생각해봤는데, 못들어가실 것 같더라고..
    flag = False                                           # 응 while 반복문 돌릴 필요도 없어~
    print(int(0))                                          # 돌아가시면 됩니다~
...
while (turn < M*N) and (flag is True):
    if check_MN[0][0] == 1:                                # 지금이 M, 즉 세로 와리가리하는 턴이니?
        for i in range(abs(M - check_MN[0][1])):           # 그러면, 이전 시행횟수 빼준 M값 만큼 반복문 돌려!
            turn += 1                                      # turn + 1 해주고
            now_idx[0] += dx[dir_turn % 4]                 # 위치 이동 가즈아~
            now_idx[1] += dy[dir_turn % 4]
            if turn == K:                                  # K 값 나오면, 중간에 멈춰!
                ans = [now_idx[1] + 1, M - now_idx[0]]     # 원하시는 출력 형식에 맞게 바꿔주고
                print(*ans)                                #
                flag = False                               # while 반복문 멈춰!
                break                                      # for 반복문 멈춰!
        ...
        for j in range(abs(N - check_MN[1][1])):           # 그러면, 이전 시행횟수 빼준 M값 만큼 반복문 돌려!
            ...
            if turn == K:                                  # K 값 나오면, 중간에 멈춰!
                ...
                flag = False                               # while 반복문 멈춰!
                break                                      # for 반복문 멈춰!
        check_MN[1][1] += 1                                # M 루트에 시행 횟수 + 1 해주자~
# < 한 사이클 종료 후에 >
