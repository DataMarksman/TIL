...
def pulling(a, b, c):
    ...
    for w_d in range(3):
        ...
        wd_list[w_d], wd_list[(w_d+1)%3] = \
            water_dist(limit_list[w_d], water_list[w_d], water_list[(w_d+1)%3])
    ...
    for w_p in range(3):
        ...
        wp_list[w_p], wp_list[(w_p + 2) % 3] = \
            water_dist(limit_list[w_p], water_list[w_p], water_list[(w_p + 2) % 3])
...
A = int(A) if A < C else int(C)
B = int(B) if B < C else int(C)
C = int(C)
limit_list = [A, B, C]
pulling(0, 0, int(C))
ans_set.remove(0)
