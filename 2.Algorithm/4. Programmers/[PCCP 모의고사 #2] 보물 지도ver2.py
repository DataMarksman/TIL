...
def solution(n, m, hole):
    # 만약 지도가 일직선인 지도라면,
    if n == 1 or m == 1:
        # 구멍이 2개 이상 일 때, 두번 뛰어넘을 수 없으므로 무조건 -1을 반환합니다.
        if len(hole) >= 2:
            return -1
        # 그 외의 경우에는 바로 옆에 있거나 2칸일 경우 1로 동일하기 때문에
        # max를 걸어주고 (일직선 거리, 1) 중에서 최적 값을 반환 해줍니다.
        else:
            return max(abs(m-n)-1, 1)

    # 좌표 기반 구멍을 조회하기 위해 set 을 만들어줍니다.
    hole_set = set()

    # 구멍을 순회하면서 SET에 넣어줍니다.
    for check_hole in hole:
        hole_set.add(tuple(check_hole))

    # 첫 값은 최대치로 주기 위해서 시스템상 max 값을 넣어줍니다.
    maxi = sys.maxsize

    # 백트래킹을 위해 각 좌표에 [점프를 쓰지 않았을 때의 최솟값, 점프를 쓰고 나서의 최솟값] 을 배정해줍니다.
    # 그 주위로 [0, 0] 으로 패딩해줍니다. DP 기반 백트래킹이기 때문에 [0, 0] 으로 패딩된 값은 진입하지 않습니다.
    # [Tip!] 0 < px <= n 과 같은 식으로 좌표 조건을 줄 수도 있습니다.
    DP = [[[0, 0]] + [[maxi, maxi] for _ in range(m)] + [[0, 0]] for _ in range(n)]
    DP = [[[0, 0] for _ in range(m + 2)]] + DP + [[[0, 0] for _ in range(m + 2)]]
    # 시작점은 [0, 0] 으로 초기화 시켜줍니다.
    DP[1][1] = [0, 0]

    # 뽑아 쓸 Q 리스트를 만들어줍니다.
    Q = []

    # 시작 값인 ( 0번 움직임, x 좌표는 1, y 좌표는 1, 점프는 쓰지 않음) 을 넣어줍니다.
    Q.append((0, 1, 1, 0))

    # 첫 값은 n*m 값, 즉 모든 값을 순회하는 값으로 넣어줍니다. 사실상 나올 수 없는 값입니다.
    answer = n * m

    # 뽑을 수 있는 Q가 있는 한 계속 돌아갑니다.
    while Q:
        # Q에서 값을 하나씩 뽑아줍니다. heapQ 기반이기 때문에 move_count 가 가장 작은 친구가 먼저 뽑힙니다.
        ...
        # 값을 넣어줄 때, (움직인 횟수, x좌표, y좌표, 점프 사용 여부) 로 넣어줬으므로 뽑은 값을 할당해줍니다.
        move_count = pick[0]
        x = pick[1]
        y = pick[2]
        jump = pick[3]
        ...
        # 만약 여기가 최종 좌표라면?
        if x == n and y == m:
            # HeapQ 기반이므로 바로 return 시켜줍니다. 처음 뽑은 값이 최적의 값입니다.
            return move_count

        # 아니라면 메인 로직에 진입합니다.
        elif move_count < answer and DP[x][y][jump] >= move_count:
            # 먼저 이번 움직임을 기록해주기 위해 move_count를 +=1 해줍니다.
            move_count += 1

            # 델타 탐색을 진행하여 4방 탐색을 시행합니다. 각각의 값의 가중치는 dx, dy에 배정해주었습니다.
