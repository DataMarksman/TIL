...
def solution(n, wires):
    ...
    root_entering[1] = sys.maxsize              # 1을 루트노드로 가정합니다. 1은 무조건 뽑히지 않도록 합니다.
    for reap_check in range(1, n+1):            # 진입 차수가 1인 리프노드들을 우선적으로 넣어줍니다.
    ...
    while pick_Q:                               # 올려 놓은 위치 값을 하나씩 뽑아 옵니다.
        ...
        for rooting in root_list[pick]:         # 해당 노드에 연결되어있는 노드들을 하나씩 순회합니다.
            ...
            root_list[rooting].remove(pick)     # 이 노드에 연결된 다른 노드들을 전부 지났으면 이 노드를 Q에 올려줍니다.
            root_entering[rooting] -= 1

            if root_entering[rooting] == 1:     # 진입차수가 1이라면 pick_Q 에 넣어줍니다.
                pick_Q.append(rooting)
                if root_count[rooting] == target_answer:
