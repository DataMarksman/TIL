...
# < 최소 공배수 함수 >
def lcm(a, b):                                                     # 최소 공배수를 구하는 재귀 함수이다.
...
for case_num in range(1, T+1):
    ...
    for write_in in range(M):                                      # 값을 받음과 동시에 위의 첫/마지막 set, 정점 리스트 기입
        ...
        if start % r == 0:                                         # 이번 정점의 시작점이 궁극의 시작점과 이어지는가?
            board[M][write_in] = 1                                 # 이어지면, 2차원 배열에 체크한다.(이전 코드의 잔상)
            first_set.add(write_in)                                # first set에도 입력하자.
        if end % c == 0:                                           # 이번 정점의 시작점이 궁극의 끝점과 이어지는가?
            board[write_in][M] = 1                                 # 이어지면, 2차원 배열에 체크한다.(이전 코드의 잔상)
    ...
    # < while 반복문: BFS 본체 >
    while n < M:                                                   # -> compare list가 True인가 여부로 최적화 가능
