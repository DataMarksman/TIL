...
def laddering(input_0, input_1, go_dir):                        # 재귀용 함수 (X좌표, Y좌표, 방향)
    dX = input_0                                                # dX 좌표
    dY = input_1                                                # dY 좌표
    ...
    while dX < 100:                                             # 밑으로 뚫고 나가지 않도록 범위 설정
        if cur_dir == 1:                                        #  < 방향이 [아래] > 일 경우
            ...
            if board[dX][dY + 1] == 2:                          # 만약 내 아래에 2가 있다면
                ...
                dY += 1                                         # 오른쪼그로 마저 진행
        return laddering(dX, dY, cur_dir)                       # 재귀용 함수에 집어넣기
    if dX == 100:                                               # 100층에 도착했다면          
        if board[dX][dY] == 2:                                  # 앞서 2로 빠지지 못한 경우를 고려
            return True                                         # 2 라면 True
        else:                                                   #
