# SWEA. 14559. 배수 그래프
# 설계 목적:
# 1. 서로가 다른 하나의 공약수에 포함되거나 서로를 곱한 수가 N 이하의 범위에 있다면, 두 점은 이어진다.
# 2. 그러므로 이렇게 이어지는 다리를 매 순간 직접 찾아가면서 정답까지 찾는다.
# 3. (1차 백트) 각 다리를 두번 이상 사용하는 것은 패턴 반복이므로, 각 다리는 한번씩만 사용한다.
# 4. (2차 백트) 중간에 이미 나왔던 종점까지의 길이보다 길게 나오게 되면 컷.
